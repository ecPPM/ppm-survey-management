...
<dialog id="newEnumeratorModal" {{ $attributes->class(['modal', 'modal-open' => $modalOpen]) }}>
    <div class="modal-box flex flex-col w-11/12 max-w-3xl px-5 md:px-10 pt-0 pb-4 md:pb-8">
        <div class="flex flex-col gap-8 relative">
            <div class="flex flex-col gap-4 pt-4 md:pt-8 bg-base-100 sticky top-0 left-0">
                <h4 class="text-lg md:text-2xl font-semibold">Create New Enumerator</h4>
                ...
                @foreach ($userAttributes as $userAttribute)
                    ...
                    @else
                        <x-survey.form-group id="{{ $userAttribute->name }}" label="{{ $userAttribute->display_text }}" is_required="{{ $userAttribute->is_required }}">
                            ...
                            >
                            <option value="" disabled selected>Select {{ $userAttribute->display_text }}</option>
                            @foreach ($userAttribute->options as $option)
            ...
            <div class="flex gap-3 items-center justify-end w-full">
                <button wire:click="clearEnumeratorCreate" class="btn btn-outline">Cancel</button>
                <button wire:click="saveEnumerator" class="btn btn-primary">Create Enumerator</button>
            </div>
        </div>
    </div>
</dialog>
